...
graph TD
    subgraph Ingestion Pipelines
        A[Scientific Papers PDF] -->|PyPDFLoader| B[Chunk 1500 / Overlap 300]
        B --> C[LLM Summarization first 10k chars]
        C -->|Inject Summary Metadata| D[(Chroma: neuroimage_methods)]
        
        E[Textbooks PDF] -->|PyPDFLoader| F[Chunk 2000 / Overlap 300]
...
* **Ingestion & Chunking:** Loaded via `PyPDFLoader` and split using `RecursiveCharacterTextSplitter` with a `chunk_size` of **1,500 characters** and a `chunk_overlap` of **300 characters**.
* **Summarization:** The first 10,000 characters of the paper (Abstract, Introduction, and early Methods) are processed by the LLM (Gemini) to generate a concise, structured global summary of the methodologies, parameters, and findings.
* **Storage & Metadata:** The global summary and a `source_type = 'Scientific Paper'` label are injected into the metadata of **every chunk** of that paper. The chunks are embedded and indexed in the `neuroimage_methods` collection.
* **Retrieval Flow:** 
