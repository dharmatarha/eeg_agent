...
4. **Execution & Final Report:** The Executor will run the code in the Docker sandbox, the Critic will review the output plots, and a final consolidated report (with decoded PNG plots written to `output/` and linked inside the markdown) will be saved to `output/final_report.md`.
